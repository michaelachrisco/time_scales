...
module TimeScales
  ...
  def self.[](hash_or_first_part, *args)
    ...
    if defines_type
      ...
      fail ArgumentError, 'Must supply only a hash argument when first argument is a hash' unless args.empty?
      ...
      fail ArgumentError, 'Must supply time-part key arguments or a single hash as arguments'
    end
  end
end
